...
// DATABASE SETTINGS
// ================================================
// Database connection settings are in database.php
...
function config($key, $default = null) {
    return defined(key)?constant(key) ? constant(
key)?constant(key) : $default;
}
...
symbol=(currency === 'KES') ? 'KSh ' : $currency . ' ';
    return symbol.numberformat(symbol . number_format(
symbol.numberf​ormat(amount, DECIMAL_PLACES);
}
...
function formatDate($date, $format = 'Y-m-d') {
    ...
    return date(format,strtotime(format, strtotime(
...
function generateReference($prefix = '', $length = 10) {
    ...
    random=strtoupper(substr(uniqid(),−(random = strtoupper(substr(uniqid(), -(
random=strtoupper(substr(uniqid(),−(length - strlen(prefix)−strlen(prefix) - strlen(
prefix)−strlen(timestamp))));
...
file_put_contents(logFile,jsonencode(logFile, json_encode(
logFile,jsone​ncode(logData) . PHP_EOL, FILE_APPEND | LOCK_EX);
}
